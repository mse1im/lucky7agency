...
import "./OtherStreamers.scss";
import { streamers, streamers2 } from "@/layout/json/other";

const OutSideStreamers: React.FC<IStreamersProps> = () => {
  const slideTrackRef = useRef<HTMLDivElement>(null);
  const slideTrackRef2 = useRef<HTMLDivElement>(null);

  const updateSlideWidthAndAnimate = (slideTrackRef: React.RefObject<HTMLDivElement>) => {
    ...
    const updateSlideWidth = () => {
      if (slideTrackRef.current) {
        ...
        slideTrackRef.current.addEventListener("mouseenter", () => animation.pause());
        slideTrackRef.current.addEventListener("mouseleave", () => animation.play());
      }
    };
    ...
    return () => {
      if (slideTrackRef.current) {
        slideTrackRef.current.removeEventListener("mouseenter", () => animation.pause());
        slideTrackRef.current.removeEventListener("mouseleave", () => animation.play());
      }
  ...
  useEffect(() => {
    const cleanUp1 = updateSlideWidthAndAnimate(slideTrackRef);
    const cleanUp2 = updateSlideWidthAndAnimate(slideTrackRef2);
  ...
  return (
    <section className="streamers-outside" id="streamers-outside">
      <Container>
        <div className="title">
          <span>Lucky 7 Agency</span>
          <h2>Diğer Yayıncılarımız</h2>
        </div>
      </Container>
      <div className="slider">
        ...
        <div className="slide-track slide-track-second" ref={slideTrackRef2}>
          {streamers2.map((streamer, index) => (
            <div
              key={index}
              className="slide"
              style={{
...
export default OutSideStreamers;
